...
```
**Group Member 1: Megan Kveragas**

**Group Member 2: Jingchuan Wang**

**Group Member 3: James Callahan**
...
regyv1v2<- lm(Y~V_1+V_2, data=out)

summary(regyv1v2)

```

The estimates are not biased because there are no missing variables.
...
Our estimate of $\beta_1$ is biased upwards due to omitting $V_2$ where we know that $V_2$ is positively correlated.
...
```{r}
out_data<- out %>% mutate(Y_adj = Y-6*V_2)

regyv1_adj<- lm(Y_adj~V_1, data=out_data)

summary(regyv1_adj)

```

Our estimate of $/beta_1$ is unbiased in this case as we have accounted for the effect of $V_2$ by adjusting $Y$.
...
**Answer:**
```{r}
plot <- ggplot(out_data, aes(V_1, y=value)) + geom_point(aes(y=Y, col="Y", alpha=0.05), color="blue") + geom_point(aes(y=Y_adj,col="Yadj", alpha=0.05), color="red") + geom_smooth(method=`lm`,aes(y=Y,col="Y", alpha=0.1),color="blue") + geom_smooth(method=`lm`,aes(y=Y_adj,col="Y",alpha=0.1),color="red") + theme_bw() + labs(x="V1 value", y="Y value/Y_adj value", title="Relationship between Y and V1, Y_adj and V1")
plot
...
#Loading data for question 4.5 & 4.6:

```{r}
datafive<-read.csv("/cloud/project/cps_clean.csv")
datafive
```

#Question 4.5:
```{r}
fourfive<-lm(inctot~edu, data=datafive)
summary(fourfive)
```


Call:
lm(formula = inctot ~ edu, data = datafive)
...
library(readr)
cps_clean <- read_csv("cps_clean.csv")
regincome<- lm(cps_clean$inctot~ cps_clean$edu)
summary(regincome)

```
The coefficient 8114.7 means the income will increase bt $8114.7 if the person has one more year of education.
...
**Answer:**
```{r}
regincome2<- lm(cps_clean$inctot~ cps_clean$edu + cps_clean$married)
summary(regincome2)
```
...
If the person is married, the one more year education will increase his/her income by 7793.1+15429.3 = 23222.4; if the person is not married, the one more year education will increase his/her income by 15429.3.
=======

```{r}
foursix<-lm(inctot~edu+age+health, data=datafive)
summary(foursix)
```

Call:
lm(formula = inctot ~ edu + age + health, data = datafive)
...
#This shows that while education is a factor, it's importance is overstated in the original regression, and other variables such as health explain some of this bias.
